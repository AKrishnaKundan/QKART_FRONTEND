...
import Cart, { getTotalCartValue, getTotalItems, generateCartItemsFrom } from "./Cart";
import "./Checkout.css";
...
const Checkout = () => {
  ...
  const addAddress = async (token, newAddress) => {
    try {
      ...
      let data = {"address":newAddress}
      const response = await axios.post(`${config.endpoint}/user/addresses`,data,header);
      setAddresses({all: response.data, selected: addresses.selected});
    } catch (e) {
  ...
  const validateRequest = (items, addresses) => {
    let cartValue = getTotalCartValue(items);
    let walletBalance = localStorage.getItem("balance");
    if (walletBalance < cartValue){
  ...
  const performCheckout = async (token, items, addresses) => {
    ...
    try{
      ...
      const response = await axios.post(`${config.endpoint}/cart/checkout`, data, header);
      let cartValue = getTotalCartValue(items);
      let newBalance = localStorage.getItem("balance")-cartValue;
      localStorage.setItem("balance", newBalance);
      history.push("/thanks",{from:"/"});
      ...
      <Grid container>
          ...
          <Box className="shipping-container" minHeight="100vh">
            ...
            <Box>
              ...
              {  (addresses.all.length === 0) ?
               ...
               <Box>
                {
                addresses.all.map((x,idx)=>{
                  if (addresses.selected === x._id){
                  return (
                  <Box className="address-item selected"  key={idx.toString()}>
                    <Typography onClick={()=>{
                      console.log(addresses);
                      setAddresses({all: addresses.all, selected: x._id});
                    }}>
                      {x.address}
                    </Typography>
                    ...
                    return (
                    <Box className="address-item not-selected"  key={idx.toString()}>
                      <Typography onClick={()=>{
                        setAddresses({all: addresses.all, selected: x._id});
                      }}>
                        {x.address}
        ...
        <Grid item xs={12} md={3}>
          ...
          <Grid container
            ...
            id="order-details"
          >
            <Grid item fontSize="1.5rem" fontWeight="700">Order Details</Grid>
            ...
            <Grid item display="flex" justifyContent="space-between" >
              <Typography fontSize="1.3rem" fontWeight="600">Total</Typography>
              <Typography fontSize="1.3rem" fontWeight="600">${getTotalCartValue(items)}</Typography>
            </Grid>
